...
@interface KxMovieViewController () {
    ...
    KxMovieGLView       *_glBackgroundView;
    PanoramaVC          *_glPanoramavc;
    ...
    UIBarButtonItem     *_gyroBtn;
    UIBarButtonItem     *_modeBtn;
...
#pragma mark KxMovie修改 Start
        _perspectMode  = ZLCPanoPerspectModeLittlePlanet; //小行星效果
        _isGyroAction  = false;
#pragma mark KxMovie修改 End
...
#pragma mark KxMovie修改Start
    //陀螺仪
     _modeIcons = @{@(ZLCPanoPerspectModeNormal):@"btn_3d_normal",@(ZLCPanoPerspectModeFisheye):@"btn_fishye_normal",@(ZLCPanoPerspectModeLittlePlanet):@"btn_littleplanet_normal"};
     _gyroIcons = @{@(true):@"btn_glyo_moving",@(false):@"btn_glyo_static"};
    
    UIButton *gyroItem = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
    [gyroItem addTarget:self action:@selector(gyroDidTouch:) forControlEvents:UIControlEventTouchUpInside];
    [gyroItem setBackgroundImage:[UIImage imageNamed:_gyroIcons[@(_isGyroAction)]] forState:UIControlStateNormal];
    
    UIButton *modeItem = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
    [modeItem addTarget:self action:@selector(modeDidTouch:) forControlEvents:UIControlEventTouchUpInside];
    [modeItem setBackgroundImage:[UIImage imageNamed:_modeIcons[@(_perspectMode)]] forState:UIControlStateNormal];
#pragma mark KxMovie修改End
...
#pragma mark KxMovie修改Start
    ...
    _gyroBtn   = [[UIBarButtonItem alloc]initWithCustomView:gyroItem];
...
#pragma mark KxMovie修改Start
    ...
    _modeBtn    = [[UIBarButtonItem alloc]initWithCustomView:modeItem];
#pragma mark KxMovie修改End
...
-(void)gyroDidTouch:(UIButton *)sender
{
    ...
    [_glPanoramavc setGyroMotioning:_isGyroAction];
...
-(void)modeDidTouch:(UIButton *)sender
{
    ...
    [_glPanoramavc setPerspectiveMode:_perspectMode];
...
- (void) setupPresentView
{
    ...
    if (_decoder.validVideo) {
        _glBackgroundView = [[KxMovieGLView alloc] initWithFrame:bounds decoder:_decoder];
#pragma mark KxMovie 添加部分 start
        _glPanoramavc = [[PanoramaVC alloc]initWithSrcType:ZLCPanoSRCTypeStream orientation:UIDeviceOrientationLandscapeRight];
        [_glPanoramavc.view setFrame:self.view.bounds];
         _glPanoramavc.view.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleBottomMargin;
        [_glPanoramavc setPerspectiveMode:_perspectMode];
        [_glPanoramavc setGyroMotioning:_isGyroAction];
        [_glBackgroundView addSubview:_glPanoramavc.view];
#pragma mark KxMovie 添加部分 end

    }
    
    if (!_glBackgroundView) {
...
- (UIView *) frameView
{
    return _glBackgroundView ? _glBackgroundView : _imageView;
}
...
#pragma mark KxMovie修改部分 start
        ...
        CVPixelBufferRef ref      = [yuvFrame asBufferRef];
        [_glPanoramavc refreshTexture:ref];
//        [_glBackgroundView render:frame];//deleted 2D
#pragma mark KxMovie修改部分 end
...
#pragma mark KxMovie修改  Start
    ...
    [_bottomBar setItems:@[_spaceItem,_gyroBtn,_fixedSpaceItem, _rewindBtn, _fixedSpaceItem, playPauseBtn,
                           _fixedSpaceItem, _fforwardBtn,_fixedSpaceItem,_modeBtn, _spaceItem] animated:NO];
#pragma mark KxMovie修改  End
...
- (void) updateHUD
{
    ...
    if (_debugAudioStatus) {
        ...
        if (NSOrderedAscending == [_debugAudioStatusTS compare: date]) {
            _debugAudioStatus = 0;
        }
    }
