...
@interface KxVideoFrameYUV()
@property (readwrite, nonatomic, assign) CVPixelBufferPoolRef pixelBufferPoolRef;
@end
@implementation KxVideoFrameYUV
- (KxVideoFrameFormat) format { return KxVideoFrameFormatYUV; }

#pragma mark KxMovie修改部分 start
- (CVPixelBufferRef)asBufferRef
{
    
    uint8_t *luma   = (uint8_t *)[self.luma bytes];
    uint8_t *chrmoB = (uint8_t *)[self.chromaB bytes];
    uint8_t *chrmoR = (uint8_t *)[self.chromaR bytes];
    if(!luma){
        return NULL;
    }
    if (self.width != self.lumaLine) {
        self.lumaLine = (int)self.luma.length/self.height;
        self.chromaBLine = self.lumaLine/2;
        self.chromaRLine = self.lumaLine/2;
    }
    CVReturn theError;
    if (!self.pixelBufferPoolRef){
        NSMutableDictionary* attributes = [NSMutableDictionary dictionary];
        [attributes setObject:[NSNumber numberWithInt:kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange] forKey:(NSString*)kCVPixelBufferPixelFormatTypeKey];
        [attributes setObject:[NSNumber numberWithInt:(int)self.width] forKey: (NSString*)kCVPixelBufferWidthKey];
        [attributes setObject:[NSNumber numberWithInt:(int)self.height] forKey: (NSString*)kCVPixelBufferHeightKey];
        [attributes setObject:@(self.lumaLine) forKey:(NSString*)kCVPixelBufferBytesPerRowAlignmentKey];
        [attributes setObject:[NSDictionary dictionary] forKey:(NSString*)kCVPixelBufferIOSurfacePropertiesKey];
        theError = CVPixelBufferPoolCreate(kCFAllocatorDefault, NULL, (__bridge CFDictionaryRef) attributes, &_pixelBufferPoolRef);
        if (theError != kCVReturnSuccess){
            NSLog(@"CVPixelBufferPoolCreate Failed");
        }
    }
    CVPixelBufferRef pixelBuffer = nil;
    theError = CVPixelBufferPoolCreatePixelBuffer(NULL, self.pixelBufferPoolRef, &pixelBuffer);
    if(theError != kCVReturnSuccess){
    ...
    memcpy(base, luma, bytePerRowY * self.height);
    base = CVPixelBufferGetBaseAddressOfPlane(pixelBuffer, 1);
    size_t   br      = bytesPerRowUV * self.height/2;
    uint8_t *brplane = (uint8_t *)base;
    for (int i = 0; i < br; i++) {
        if (i%2 == 0)
        {
            brplane[i] = chrmoB[i/2];
        }else
        {
            brplane[i] = chrmoR[i/2];
        }
    }
    CVPixelBufferUnlockBaseAddress(pixelBuffer, 0);
    CVPixelBufferPoolRelease(self.pixelBufferPoolRef);
    return pixelBuffer;
}
...
- (KxVideoFrame *) handleVideoFrame
{
    ...
    if (_videoFrameFormat == KxVideoFrameFormatYUV) {
        ...
        yuvFrame.chromaR = copyFrameData(_videoFrame->data[2],
                                         ...
                                         _videoCodecCtx->width / 2,
                                         _videoCodecCtx->height / 2);
#pragma mark KxMovie 修改部分 start
        yuvFrame.lumaLine     = _videoFrame->linesize[0];
        yuvFrame.chromaBLine  = _videoFrame->linesize[1];
        yuvFrame.chromaRLine  = _videoFrame->linesize[2];
#pragma mark KxMovie 修改部分 end
